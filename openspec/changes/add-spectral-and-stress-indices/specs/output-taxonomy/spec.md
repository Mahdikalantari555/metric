...
### Requirement: Six-stage linear pipeline (pipeline-architecture)

**FROM**:
```
3. calculate_surface_properties() — NDVI/EVI/LAI/SAVI/FVC, albedo, emissivity, LST, roughness
```

**TO**:
```
3. calculate_surface_properties() — NDVI/EVI/LAI/SAVI/FVC, albedo, emissivity, LST, roughness
3b. calculate_spectral_indices() — NDMI, MSI, NMDI, NIRv, GCI, NDSI, SI_T (per-scene)
```

Stage 3b SHALL run after stage 3 and before stage 4
(`calculate_radiation_balance`). It SHALL read bands already present in the
cube (`nir08`, `swir16`, `swir22`, `green`, `red`) and add the new index
bands to the cube. Missing bands SHALL cause the stage to skip silently
(with a warning log), matching the existing graceful-failure pattern.

#### Scenario: Stage 3b computes NDMI when bands present
- **WHEN** `nir08` and `swir16` are in the cube
- **THEN** `ndmi` is added to the cube during stage 3b

#### Scenario: Stage 3b skips gracefully on missing band
- **WHEN** `swir22` is missing (e.g. Landsat 7 without SWIR2)
- **THEN** NMDI is skipped with a warning; other indices still compute
